...
@pgf axis = SemiLogXAxis(
    {
        xlabel = "Overall Rate of Coupling and Uncoupling / \$s^{-1}\$",
...
for k_couple in k_couples
    v_eff_transcriptions = (L)./df.T_transcription[df.k_couple .== k_couple]
    v_eff_translations = (L)./df.T_translation[df.k_couple .== k_couple]
...
plot_line = Plot(Coordinates(k_couples, v_eff_transcription; yerror= std_eff_transcription ))
legend_line = LegendEntry("transcription")
push!(axis, plot_line )
push!(axis, legend_line )
plot_line = Plot(Coordinates(k_couples, v_eff_translation; yerror = std_eff_translation))
...
for k_couple in k_couples
    T_exposures = df.T_exposure[df.k_couple .== k_couple]
    T_exposures_uncoupled = df.T_exposure_uncoupled[df.k_couple .== k_couple]
...
plot_line = Plot(Coordinates(k_couples, mean_T_exposure; yerror= std_T_exposure ))
legend_line = LegendEntry("exposure duration")
push!(axis, plot_line )
push!(axis, legend_line )
plot_line = Plot(Coordinates(k_couples, mean_T_exposure_uncoupled; yerror = std_T_exposure_uncoupled))
...
plot_line = Plot(Coordinates(k_couples, fractions_T_exposure; yerror= std_fractions_T_exposure ))
legend_line = LegendEntry("exposure fraction")
push!(axis, plot_line )
push!(axis, legend_line )
plot_line = Plot(Coordinates(k_couples, fractions_T_exposure_uncoupled; yerror = std_fractions_T_exposure_uncoupled))
...
for k_couple in k_couples
    distances = df.mean_distance[df.k_couple .== k_couple]
    push!(mean_distances, mean(distances))
    push!(std_distances, std(distances))
end
plot_line = Plot(Coordinates(k_couples, mean_distances; yerror= std_distances ))
