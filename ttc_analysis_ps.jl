...
if isfile("analysis_$(label).csv") & append_flag
    df = CSV.read("analysis_$(label).csv",DataFrame)
    cmds = []
    for p in ps
    ...
    CSV.write("analysis_$(label).csv",df)
elseif isfile("analysis_$(label).csv") & !overwrite    
    df = CSV.read("analysis_$(label).csv",DataFrame)
else
    cmds = []
    for p in ps
        cmd = `julia ttc_simu_analysis.jl $k $p $L $ℓ $k_translation_initiation $k_transcription_termination $Eᵦ $E_c $k_couple $k_stalling_0 $k_unstalling_0 $k_ini_pausing $d $type $mode $N`
        push!(cmds, cmd)
    end
    @showprogress 1 pmap(run, cmds)
    df = DataFrame(
    ...
    CSV.write("analysis_$(label).csv",df)
        ...
        for p in ps
            ...
            T0s = df.T_0[df.v_translations .== p]
            T1s = df.T_1[df.v_translations .== p]
        ...
        CSV.write("fig/analysis_df_$(label).csv",plot_df)
